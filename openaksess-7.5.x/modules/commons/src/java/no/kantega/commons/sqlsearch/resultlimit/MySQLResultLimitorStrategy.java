 ...
 *
 */
public class MySQLResultLimitorStrategy extends ResultLimitorStrategyAdapter {
    public String limitAfterOrder(SearchTerm searchTerm) {
        if(searchTerm.getFirstResult() != -1) {
            String limit = "LIMIT " + searchTerm.getFirstResult();
            if(searchTerm.getMaxResults() != -1) {
                limit += "," +searchTerm.getMaxResults();
            }
            return limit;
        } else {
