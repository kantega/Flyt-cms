...
public class PrintFormadminNavigatorTag extends PrintNavigatorTag {
    ...
    protected void printBody(NavigationMapEntry item) throws IOException {
        FormadminMapEntry currentItem = (FormadminMapEntry)item;
        ...
        String type = "";
        if (currentItem.getObjectType() == FormadminObjectType.FORM_TYPE_INSTANCE.ordinal()) {
            type = "instance";
        } else if (currentItem.getObjectType() == FormadminObjectType.ROOT.ordinal()) {
            type = "root";
        } else {
            type = "state";
        }
