...
public class UploadMultimediaAction extends AdminController {
    private static String SOURCE = "aksess.EditMultimediaAction";
    private String view;
    ...
    public ModelAndView handleRequestInternal(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        for (Multimedia m : multimedia) {
            ...
            if (mimeType.shouldConvertImage() && Aksess.isImageConversionEnabled()) {
                // Image conversion is enabled, convert to image format readable by browsers
                m.setData(MultimediaHelper.convertImageFormat(m.getData()));
            ...
            // Save object
            newId = mediaService.setMultimedia(m);

        }

        // Todo: Maybe this should be in a popup or ajax or something?
        Map model = new HashMap();
        if (multimedia.size() == 1 && newId != -1) {
            model.put("id", newId);
            return new ModelAndView(new RedirectView("EditMultimedia.action"));
        } else {
